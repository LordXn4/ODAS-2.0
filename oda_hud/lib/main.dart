...
enum OdaState { idle, listening, processing }
...
class _OdaHudState extends State<OdaHud> with SingleTickerProviderStateMixin {
  ...
  void _connectHud() {
    try {
      socket = WebSocketChannel.connect(Uri.parse('ws://192.168.18.4:8766'));
  ...
  Future<void> speak(String text) async {
    ...
    try {
      await voiceChannel.invokeMethod('speak', {'text': text});
    } catch (e) {
...
class OdaHudPainter extends CustomPainter {
  ...
  void _background(Canvas canvas, Size size) {
    final shader =
        const RadialGradient(
          colors: [Color(0xFF18052B), Color(0xFF080311), Color(0xFF010106)],
          stops: [0, .45, 1],
        ).createShader(
          Rect.fromCenter(
  ...
  void _particles(Canvas canvas, Offset center, double radius) {
    for (int i = 0; i < 110; i++) {
      final angle = i * 2.399963 + progress * motion * (i.isEven ? .95 : -.70);

      final baseDistance = radius * (.58 + ((i * 47) % 100) / 190);

      final audioOffset = audioLevel * radius * .055 * math.sin(i * 1.7);

      final distance = baseDistance + audioOffset;

      final x = center.dx + math.cos(angle) * distance;
      final y = center.dy + math.sin(angle) * distance * .58;

      final particleSize = .45 + ((i * 17) % 6) * .28;

      final alpha = (.10 + (i % 5) * .065) * intensity;
  ...
  void _radialStructure(Canvas canvas, Offset center, double radius) {
    final paint = Paint()
      ..color = const Color(0xFFA64DFF).withValues(alpha: .13 * intensity)
      ..strokeWidth = .55;

    for (int i = 0; i < 48; i++) {
      final angle = i * math.pi * 2 / 48 + progress * math.pi * 2 * .18;

      final inner = radius * .74;
      final outer = radius * (.90 + (i % 4) * .035);
  ...
  void _outerHud(Canvas canvas, Offset center, double radius) {
    ...
    final paint = Paint()
      ...
      ..strokeWidth = .8
      ..color = const Color(0xFFB35CFF).withValues(alpha: .22 * intensity);

    canvas.drawOval(rect, paint);

    paint
      ..strokeWidth = 1.6
      ..color = const Color(0xFFD09AFF).withValues(alpha: .62);

    final start = progress * math.pi * 2 * .35;

    canvas.drawArc(rect, start, math.pi * .38, false, paint);

    canvas.drawArc(rect, start + math.pi * 1.18, math.pi * .18, false, paint);

    for (int i = 0; i < 8; i++) {
      final a = progress * math.pi * 2 * .12 + i * math.pi / 4;
  ...
  void _orbits(Canvas canvas, Offset center, double radius) {
    for (int i = 0; i < 5; i++) {
      ...
      final paint = Paint()
        ...
        ..color = const Color(
          0xFFB963FF,
        ).withValues(alpha: (.15 + i * .025) * intensity);
      ...
      canvas.rotate(
        progress * math.pi * 2 * (i.isEven ? 1 : -1) * (0.45 + i * .07),
      );
      ...
      final nodeAngle = progress * math.pi * 2 * (i + 1) * .45 + i * 1.7;

      final nodeX = center.dx + math.cos(nodeAngle) * r;

      final nodeY = center.dy + math.sin(nodeAngle) * r * .56;

      final nodePaint = Paint()
        ..color = const Color(0xFFE0B6FF).withValues(alpha: .72);

      canvas.drawCircle(Offset(nodeX, nodeY), i == 2 ? 2.5 : 1.7, nodePaint);
    }
  }

  void _wave(Canvas canvas, Offset center, double radius) {
    final path = Path();
    const points = 220;
    ...
    for (int i = 0; i <= points; i++) {
      ...
      final wave =
          math.sin(angle * 8 + progress * math.pi * 12 * motion) *
      ...
      final y = center.dy + math.sin(angle) * r * .56;
    ...
    final paint = Paint()
      ..style = PaintingStyle.stroke
      ..strokeWidth = state == OdaState.idle ? 1.0 : 1.35
      ..color = const Color(
        0xFFD6A4FF,
      ).withValues(alpha: state == OdaState.idle ? .40 : .58);

    canvas.drawPath(path, paint);
  }

  void _core(Canvas canvas, Offset center, double radius) {
    final baseRadius = radius * .115;

    final subtlePulse =
        math.sin(progress * math.pi * 2 * motion) * radius * .012;
    ...
    final glow = Paint()
      ..shader = RadialGradient(
        colors: [
          const Color(0xFFF2DDFF).withValues(alpha: .40),
          const Color(0xFFC45CFF).withValues(alpha: .22),
          const Color(0xFF7B16B2).withValues(alpha: .10),
          Colors.transparent,
        ],
        stops: const [0, .20, .55, 1],
      ).createShader(Rect.fromCircle(center: center, radius: r * 4.8));

    canvas.drawCircle(center, r * 4.8, glow);
    ...
    final halo = Paint()
      ..style = PaintingStyle.stroke
      ..strokeWidth = .8
      ..color = const Color(0xFFE9C9FF).withValues(alpha: .60);

    canvas.drawCircle(center, r * 1.55, halo);

    final arc = Paint()
      ..style = PaintingStyle.stroke
      ..strokeWidth = 1.0
      ..strokeCap = StrokeCap.round
      ..color = const Color(0xFFC45CFF).withValues(alpha: .65);

    final arcRect = Rect.fromCircle(center: center, radius: r * 2.05);

    canvas.drawArc(arcRect, progress * math.pi * 2, math.pi * .72, false, arc);
  ...
  void _avatarFrame(Canvas canvas, Offset center, double radius) {
    final rect = Rect.fromCenter(
      center: center,
      width: radius * 1.05,
      height: radius * 1.30,
    ...
    final paint = Paint()
      ..style = PaintingStyle.stroke
      ..strokeWidth = .45
      ..color = const Color(0xFFD8A8FF).withValues(alpha: .10);
